...
import { findEarliestNextRunAtMs, MAX_TIMER_DELAY_MS, planTimerDelayMs } from "../src/cron/timer.js";
...
describe("cron timer", () => {
  it("caps timer delays at sixty seconds", () => {
    expect(planTimerDelayMs(Date.parse("2026-04-16T09:02:30.000Z"), Date.parse("2026-04-16T09:00:00.000Z"))).toBe(
      MAX_TIMER_DELAY_MS,
    );
  });

  it("fires overdue work immediately", () => {
    expect(planTimerDelayMs(Date.parse("2026-04-16T08:59:59.000Z"), Date.parse("2026-04-16T09:00:00.000Z"))).toBe(0);
  });

  it("uses the exact remaining delay when it is under the cap", () => {
    expect(planTimerDelayMs(Date.parse("2026-04-16T09:00:45.000Z"), Date.parse("2026-04-16T09:00:00.000Z"))).toBe(
      45_000,
    );
  });
  ...
  it("returns undefined when no enabled jobs are scheduled", () => {
    expect(findEarliestNextRunAtMs([createJob("disabled", 1, false), createJob("missing")])).toBeUndefined();
  });
});
